...
A simple program that bruteforces tinyurl combinations to find working ones
Maybe tinyurl should increase amount of random letters & numbers?
...
class checker(object):
    ...
    def run(self):
        '''
        Asks for user input and then tries to generate as many tinyurls as requested
        '''
        self.working_urls = []
        ...
        while(len(self.working_urls)<amount):
            ...
            working = self.check_this("http://www.tinyurl.com/" + generated_string)
            if(working):
                current+=1
            print("-------------------")
    ...
    def read_html_file(self,file):
        '''
        Checks what content there is on a site that was found
        '''
        html = open(file)
        line_contains = ""
